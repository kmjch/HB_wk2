...
def translate_to_pirate_talk(phrase):
    ...
    # made another python file 'make_dict.py' and txt file 'wordfile.txt' to
    # generate dictionary that I can read through in this function
    eng_pir = {'sir': 'matey', 'restaurant': 'galley', 'students': 'swabbies',
               'professor': 'foul blaggart', 'hotel': 'fleabag inn',
               'restroom': 'head', 'excuse': 'arr', 'are': 'be',
               'student': 'swabbie', 'my': 'me', 'your': 'yer', 'is': 'be',
               'man': 'matey'}

    words = phrase.split(' ')
    translated_sent = ''
    for word in words:
        if word in eng_pir.keys():
            word = eng_pir[word]
        translated_sent += word + ' '
    if translated_sent[len(translated_sent) - 1] == ' ':
        translated_sent = translated_sent[:len(translated_sent) - 1]
